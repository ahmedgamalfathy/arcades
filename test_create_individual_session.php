...
echo "=== Creating Individual Session (Type 0) with BookedDevice ===\n\n";
...
echo "Device Time: {$deviceTime->name} (ID: {$deviceTime->id}, Rate: {$deviceTime->rate})\n\n";
...
$session = App\Models\Timer\SessionDevice\SessionDevice::create([
    'name' => 'individual', // Individual sessions usually have generic name
    'daily_id' => 2,
    'type' => 0, // INDIVIDUAL
]);
echo "✓ Session created (ID: {$session->id}, Type: {$session->type})\n\n";

// Step 2: Create BookedDevice for this session
echo "Step 2: Create BookedDevice\n";
...
echo "  Type: {$deviceType->name}\n";
echo "  Time: {$deviceTime->name}\n";
